<aside id="sidebar" class="fixed inset-y-0 left-0 z-50 flex w-[240px] flex-col bg-[#003F87] text-white transition-transform duration-300 md:relative md:translate-x-0 -translate-x-full">
    ...
    <div class="flex h-14 items-center justify-between border-b border-white/10 px-5 shrink-0">
        ...
        <button onclick="toggleSidebar()" class="text-white/60 hover:text-white md:hidden cursor-pointer" aria-label="Collapse sidebar">
            <i class="ti ti-x text-lg"></i>
        </button>
        <span class="rounded-full bg-white/10 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-white/70">Admin</span>
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4 scrollbar-thin scrollbar-thumb-white/10">
        <button onclick="switchScreen('overview')" data-nav="overview"
                class="flex w-full items-center gap-3 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-white transition-all duration-200 bg-[#0057BD] rounded-md cursor-pointer text-left">
            <i class="ti ti-layout-dashboard text-[20px]"></i>
            Overview / Dashboard
        </button>

        <button onclick="switchScreen('buses')" data-nav="buses"
                class="flex w-full items-center gap-3 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-white/70 hover:text-white hover:bg-white/[0.04] transition-all duration-200 rounded-md cursor-pointer text-left">
        ...
        <button onclick="switchScreen('routes')" data-nav="routes"
                class="flex w-full items-center gap-3 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-white/70 hover:text-white hover:bg-white/[0.04] transition-all duration-200 rounded-md cursor-pointer text-left">
        ...
        <button onclick="switchScreen('analytics')" data-nav="analytics"
                class="flex w-full items-center gap-3 px-4 py-2.5 text-xs font-semibold uppercase tracking-wider text-white/70 hover:text-white hover:bg-white/[0.04] transition-all duration-200 rounded-md cursor-pointer text-left">
