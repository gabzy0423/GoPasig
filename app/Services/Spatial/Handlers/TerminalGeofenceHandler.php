...
use App\Events\BusEnteredTerminal;
use App\Events\BusExitedTerminal;

class TerminalGeofenceHandler implements GeofenceHandlerInterface
{
    public function handle(VehiclePosition $position, Geofence $geofence, SpatialStateResult $result, ?Trip $trip): void
    {
        ...
        if ($result->state === SpatialPresenceState::ENTERING) {
            ...
            // 2. Dispatch events
            event(new BusEnteredGeofence($bus->id, $tripId, $geofence->id, 'TERMINAL'));
            event(new BusEnteredTerminal($bus, $geofence, $result->distance, $now, $position));
            ...
            if ($activeTransition) {
                ...
                // Dispatch events
                event(new BusExitedGeofence($bus->id, $tripId, $geofence->id, 'TERMINAL'));
                event(new BusExitedTerminal($bus, $geofence, $result->distance, $now, $position));
            }
        }
    }
}
