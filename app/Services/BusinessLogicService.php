...
class BusinessLogicService
{
    ...
    ): array
    {
        ...
        $schedules = Schedule::where('driver_id', $driverId)
            ...
            ->whereNotIn('status', ['cancelled', Schedule::STATUS_CANCELLED])
    ...
    public static function checkScheduleConflict(
        ...
        foreach ($existingSchedules as $schedule) {
            ...
            if ($schedule->bus_id === $busId) {
                if ($newStart->lessThan($sEnd->copy()->addMinutes($busBuffer)) &&
            ...
            if ($schedule->driver_id === $driverId) {
                if ($newStart->lessThan($sEnd->copy()->addMinutes($driverBuffer)) &&
